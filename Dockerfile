...
COPY --from=buildstage $APP_HOME/build/libs/bike-race-timecut-calculator-0.0.1-SNAPSHOT.jar $APP_HOME/bike-race-timecut-calculator.jar
# Set the working directory to /app, so we don't need to prefix the CMD Layer with /app.
WORKDIR $APP_HOME
# Expose port 8080
EXPOSE 8080
# Start the Spring Boot app
CMD ["java", "-jar", "bike-race-timecut-calculator.jar"]
